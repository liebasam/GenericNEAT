...
{
    /// <summary>
    /// A genetic algorithm, largely taken from <see cref="GeneticSharp.Domain.GeneticAlgorithm"/>.
    /// </summary>
    public sealed class SpeciedGeneticAlgorithm : IGeneticAlgorithm
    {
        #region Constants
        /// <summary>
        ...
        public SpeciedGeneticAlgorithm(
        ...
        public ISpeciedPopulation Population { get; private set; }
        ...
        /// <summary>
        /// Reorders the population and its species so that chromosomes
        /// appear in decending order by fitness.
        /// </summary>
        private void ReorderChromosomes()
        {
            var ordered = Population.CurrentGeneration.Chromosomes.OrderByDescending(c => c.Fitness.Value).ToList();
            Population.CurrentGeneration.Chromosomes.Clear();
            ordered.ForEach(c => Population.CurrentGeneration.Chromosomes.Add(c));
            foreach (var specie in Population.Species)
            {
                ordered = specie.CurrentGeneration.Chromosomes.OrderByDescending(c => c.Fitness.Value).ToList();
                specie.CurrentGeneration.Chromosomes.Clear();
                ordered.ForEach(c => specie.CurrentGeneration.Chromosomes.Add(c));
            }
        }
